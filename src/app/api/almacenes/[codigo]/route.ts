...
export async function PUT(
  ...
  try {
    ...
    if (body.direccion) {
      ...
      const errorDireccion = validarDireccion(camposDireccion as DireccionInput);
      if (errorDireccion) {
        return NextResponse.json({ error: errorDireccion }, { status: 400 });
      }
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ message: "Almacén eliminado correctamente" });
  } catch (error: unknown) {
    return NextResponse.json(
      {
